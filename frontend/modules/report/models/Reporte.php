    ...
    public function behaviors()
{
     ...
     */
    public function putCabecera($id,$idfiltro,$campofiltro=null,$ultima=false){        
     ...
     foreach( $hijosCabecera as $record) {
          // var_dump($this->modelToRepor($idfiltro));die();
                if(!($record->lastpage && $ultima===false)){
                 $HTML_cabecera.=$record->putStyleField($record->nombre_campo,$modelin->{$record->nombre_campo});    
                }
		 
               }
         unset($modelin);   unset($modeloToReport);unset($hijosCabecera);unset($clase);
   ...
   */
    public function makeColumns($idfiltro,$valorAcumulado=null){
        ...
        foreach($hijosDetalle as $fila){
            $elementos=[
               ...
               // 'group'=>($fila['agrupacion']=='1')?true:false,
               'format'=>(($fila['esnumerico']=='1') && !is_null($fila['nombre_campo']))?['decimal',2]:'raw',
            ...
            if($fila['totalizable']=='1'){
               ...
               if($fila['esnumerico']=='1'){
                 $elementos=array_merge($elementos,['footer' => '<div style="font-weight:bold; text-align:right !important;">'
           .round((is_null($valorAcumulado))?0:$valorAcumulado,2).'</div>']);
                   } else{
    ...
    public function dataProvider($idfiltro){
       ...
       if(count($campoOrden)>0){
           $sorter=['sort' => 
               [
                 'defaultOrder' => [                          
                           $campoOrden[0] => SORT_ASC, 
                               ] 
                ]];
           $options= array_merge($options,$sorter);
       }
       //var_dump($options);die();
        
                $provider = new \yii\data\ActiveDataProvider($options);
                return $provider;
                  }
    ...
    public function creaReporte($id, $idfiltro){
       Yii::$app->controller->layout='@frontend/modules/report/views/layouts/blank';
     ...
     private function McontentReport($id, $idfiltro){
      ...
      for($i = 1; $i <= $npaginas; $i++){
         ...
         $pageContents[]=trim(Yii::$app->controller->render('@frontend/modules/report/views/make/reporte',[
             'modelo'=>$this,             
   ...
   private function MputLogo($id, $idfiltro){               
        return Yii::$app->controller->renderpartial('@frontend/modules/report/views/make/logo',
				array(
  ...
  private function MprepareFormat($contenido){
     ...
     /*
      * Devuelve un array co las suma por cada pagina 
      * muy util para ir mostrando los acumulados 
      */   
   public function sumDataProvider($dataprovider){
     $camposTotalizables=$this->camposTotalizables();
     if(count($camposTotalizables)>0){       
           $models=$dataprovider->getModels();
           $nombrecampo=$camposTotalizables[0];
              $valor=0;
             foreach($models as $model){
                 $valor=$model->{$nombrecampo}+$valor;
                 }
               return $valor; 
     }else{
       return 0;  
  ...
  public function camposTotalizables(){
     return  $this->getReportedetalle()->select(['nombre_campo'])->
    andWhere(['esdetalle'=>'1','totalizable'=>'1'])->column();
  }
  
  public function campoOrden(){
    return  $this->getReportedetalle()->select(['nombre_campo'])->
    andWhere(['esdetalle'=>'1','orden'=>1])->column();  
  }
}