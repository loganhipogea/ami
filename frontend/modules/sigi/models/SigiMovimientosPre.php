...
class SigiMovimientosPre extends \common\models\base\modelBase {
    ...
    public function rules() {
        return [
            ...
            [['monto_conciliado', 'resumen_id'], 'safe'],
    ...
    public function beforeSave($insert) {
        if ($insert) {
            ...
            IF (empty($this->fechaop))
                $this->fechaop = self::SwichtFormatDate(self::CarbonNow()->format(\common\helpers\timeHelper::formatMysqlDate()), 'date', true);
            if ($this->isKardex()) {
    ...
    public function validate_monto_fraccionado($attribute, $params) {
       if($this->activo){//SI YA ESTA APROBADO, Y SE ESTA MODIFICANDO ES PÒRQUE ESE ESTA BORRANDO NO AHY TROA POSIBILIDAD
        $movBanco = $this->movBanco;
        yii::error('EL MONTO DE ESTE MOCI MIETO, NO DEL BANCO ',__FUNCTION__);
        yii::error($this->monto,__FUNCTION__);
        
        yii::error('monto_concialido',__FUNCTION__);
        $montoconciliado = $movBanco->montoConciliado();
        yii::error($montoconciliado,__FUNCTION__);
        $signo = ($this->monto > 0) ? 1 : -1;
        //if($this->isNewRecord){

        $diferencia = abs(abs($movBanco->monto) - abs($montoconciliado));
        yii::error('DIFERENCIA   monto(mov banco) -  monto concialiado',__FUNCTION__);
       
        yii::error($diferencia,__FUNCTION__);
        if (abs($this->monto) > $diferencia){
            yii::error('abs(this->monto) > diferencia',__FUNCTION__);
        ...
        if ($this->isKardex()) {
              yii::error('Es kardex',__FUNCTION__);
            $kardex = $this->kardex;
            if ($this->monto > $kardex->deuda()) {
               yii::error('El monto this->monto > kardex-deuda()',__FUNCTION__); 
                yii::error('El monto '.$this->monto.' > '.$kardex->deuda().' ',__FUNCTION__); 
