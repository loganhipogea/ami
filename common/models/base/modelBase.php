     ...
     return $retorno;
       ...
       public static function SwichtFormatDate($valor,$tipo,$show){
        ...
        public function prepareTimeFields($show){
            if($this->convertTimes){ //para bypassear este compertamiento
...
public function firstMessage($category=null){
  ...
  public function isDateOrTimeField($attribute){
      return in_array($attribute, array_keys($this->dateorTimeFields));
  }
  
  /*
   * erifica si es 01-01-1970'
   */
 public function dateIsInicial($attribute){
     if($this->isDateOrTimeField($attribute)){
         if($this->toCarbon($attribute)->gt(Carbon::createFromFormat(\common\helpers\timeHelper::formatMysqlDate(), \common\helpers\timeHelper::getDateInitial()))){
             RETURN FALSE;
         }ELSE{
             RETURN TRUE;
         }
